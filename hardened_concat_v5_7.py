...
def suggest_mpv_flags(gpu):
    if gpu["vendor"] == "intel" and gpu["generation"] in ("ivybridge", "sandybridge"):
        return "--vo=gpu --scale=bilinear --correct-downscaling=no"
    if gpu["vendor"] == "intel":
...
# ------------------------------------------------------------
# FAST RE-ENCODE (HandBrakeCLI, QuickSync, or ultrafast)
# ------------------------------------------------------------
def fast_reencode(input_file, output_file):
    ...
    # always fallback to ultrafast software encode
...
def main():
    if len(sys.argv) < 3:
        print("Usage: v5_7.py output.mp4 input1.mp4 [input2.mp4 ...] [--force-ts]")

#!/usr/bin/env python3
"""
Lossless FFmpeg concat tool v5.12
- Interactive mode (run without arguments or with --interactive)
- All features from v5.11: --check-only, disk space, auto TS fallback, --dry-run, save/load project, progress bar
"""
...
# ------------------------------------------------------------
# RUNNER – with optional progress bar
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# INTERACTIVE ORDERING (full wizard)
# ------------------------------------------------------------
def interactive_wizard():
    print("\n" + "="*60)
    print("INTERACTIVE MODE - Lossless Concat Tool v5.12")
    print("="*60)

    # Output file
    output = input("Output file name [merged.mp4]: ").strip()
    if not output:
        output = "merged.mp4"
    ...
    # Input files
    print("\nEnter input files (space-separated, or use glob patterns like '*.mp4' or '~/Videos/*.MP4')")
    files_raw = input("Files: ").strip()
    if not files_raw:
        print("No files provided. Exiting.")
        sys.exit(1)

    # Expand glob patterns and split
    files = []
    ...
    files = list(dict.fromkeys(files))  # preserve order
    ...
    # Options
    print("\nOptions (press Enter for default 'no' or 'auto'):")
    force_ts = input("Force TS fallback? [y/N]: ").strip().lower() == 'y'
    force_direct = input("Force direct concat (override risky detection)? [y/N]: ").strip().lower() == 'y'
    dry_run = input("Dry run (only show commands, no execution)? [y/N]: ").strip().lower() == 'y'
    check_only = input("Only run compatibility check? [y/N]: ").strip().lower() == 'y'
    progress = input("Show progress bar during re-encode? [y/N]: ").strip().lower() == 'y'

    # Confirm
    print("\n" + "="*60)
    print("SUMMARY:")
    print(f"Output: {output}")
    print(f"Files: {len(files)} inputs")
    print(f"Force TS: {force_ts}")
    print(f"Force direct: {force_direct}")
    ...
    return output, files, force_ts, force_direct, dry_run, check_only, progress
...
# ------------------------------------------------------------
# MAIN – with interactive mode detection
# ------------------------------------------------------------
def main():
    check_deps()

    # If no arguments or --interactive, go into interactive wizard
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] == "--interactive"):
        output, files, force_ts, force_direct, dry_run, check_only, progress = interactive_wizard()
    else:
        # Normal argument parsing (same as v5.11)
        import argparse
        parser = argparse.ArgumentParser(description="Lossless FFmpeg concat tool v5.12")
        parser.add_argument("output", nargs="?", help="Output file (required unless --interactive)")
        parser.add_argument("inputs", nargs="*", help="Input files")
        parser.add_argument("--force-ts", action="store_true", help="Force TS fallback")
        parser.add_argument("--force-direct", action="store_true", help="Force direct concat")
        parser.add_argument("--dry-run", action="store_true", help="Print commands and estimate")
        parser.add_argument("--check-only", action="store_true", help="Only run compatibility report")
        parser.add_argument("--progress", action="store_true", help="Show progress bar during re-encode")
        parser.add_argument("--save", metavar="FILE", help="Save project to JSON")
        parser.add_argument("--load", metavar="FILE", help="Load project from JSON")
        parser.add_argument("--interactive", action="store_true", help="Run interactive wizard (ignores other args)")
        args = parser.parse_args()

        if args.load:
            proj = json.load(open(args.load))
            files = proj["files"]
            output = proj["output"]
            ...
            if not args.output:
                parser.error("output file required (unless --load or --interactive)")
            output = args.output
            files = args.inputs
        ...
        if args.save:
            ...
            print(f"Project saved to {args.save}")

    # At this point we have output, files, and flags
    if check_only:
